...
data_PC_GA = data_extract_GA.data.PC;
data_MC_GA = data_extract_GA.data.MC;
data_EC_GA = data_extract_GA.data.EC;

data_PC_GB = data_extract_GB.PC;
data_MC_GB = data_extract_GB.MC;
...
h1 = boxplot(mean_score_GA(:,3), 'positions', x1, 'colors', 'b');
hold on
h2 = boxplot(mean_score_GB(:,3), 'positions', x2, 'colors', 'r');

% Modifying line widths
set(findobj(h1, 'type', 'line'), 'LineWidth', 1.5); 
set(findobj(h2, 'type', 'line'), 'LineWidth', 1.5); 

% Text annotations with Times New Roman font
text(x1, mean(mean_score_GA(:,3)) + 5, sprintf('%.2f', mean(mean_score_GA(:,3))), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontSize', 14, 'Color', 'b', 'FontName', 'Times New Roman');
text(x2, mean(mean_score_GB(:,3)) - 5, sprintf('%.2f', mean(mean_score_GB(:,3))), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontSize', 14, 'Color', 'r', 'FontName', 'Times New Roman');
text(1.33, 70, sprintf('%s', 'p-value:'), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontSize', 14, 'Color', 'k', 'FontName', 'Times New Roman');
text(1.35, 65, sprintf('%s', 'Reject Null:'), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontSize', 14, 'Color', 'k', 'FontName', 'Times New Roman');
text(1.5, 70, sprintf('%.2g', p_EC), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontSize', 14, 'Color', 'k', 'FontName', 'Times New Roman');
text(1.5, 65, sprintf('%s', 'Yes'), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontSize', 14, 'Color', 'k', 'FontName', 'Times New Roman');
...
% end
% for i = 1:numel(pos_2)
%     text(pos_2(i), mean(mean_score_GB(:,i)) - 5, sprintf('\\mu = %.2f', mean(mean_score_GB(:,i))), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontSize', 10, 'Color', 'r');
% end
% 
% ylabel('Score %')
% axis square; % Make axis square
...
% ylim([-1, 30]) % Adjust the y-axis limit as needed
% legend('Group A', 'Group B', 'FontSize', 24) 
% 
% sgtitle('Analysis Comparison', 'FontSize', 24, 'FontName', 'Times New Roman')
