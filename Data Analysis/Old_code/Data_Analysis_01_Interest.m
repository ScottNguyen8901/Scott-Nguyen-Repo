...
data_extract_GA = importdata('G:\My Drive\UIUC\Spring 2024\DodNDEP\Results\Data Analysis\Group A\Interest Assessment.xlsx');

data_extract_PC_GB = importdata('G:\My Drive\UIUC\Spring 2024\DodNDEP\Results\Data Analysis\Group B\Interest Assessment PC.csv');
data_extract_MC_GB = importdata('G:\My Drive\UIUC\Spring 2024\DodNDEP\Results\Data Analysis\Group B\Interest Assessment MC.csv');
data_extract_EC_GB = importdata('G:\My Drive\UIUC\Spring 2024\DodNDEP\Results\Data Analysis\Group B\Interest Assessment EC.csv');

data_PC_GA = str2double(data_extract_GA.PC);
data_MC_GA = str2double(data_extract_GA.MC);
data_EC_GA = str2double(data_extract_GA.EC);

data_PC_GB = data_extract_PC_GB.data;
data_MC_GB = data_extract_MC_GB.data;
data_EC_GB = data_extract_EC_GB.data;
...
if H_PC_GA == 0 && H_PC_GB == 0
    ...
    [p_PC, h_PC, stats_PC] = ranksum(median(data_PC_GA,2), median(data_PC_GB,2), 'tail', 'right', 'alpha', 0.01);
    test_PC = 1;
end
...
if H_MC_GA == 0 && H_MC_GB == 0
    ...
    % Perform ranksum test
    [p_MC, h_MC, stats_MC] = ranksum(median(data_MC_GA,2), median(data_MC_GB,2), 'tail', 'right', 'alpha', 0.01);
    test_MC = 1;
end
...
if H_EC_GA == 0 && H_EC_GB == 0
    ...
    % Perform ranksum test
    [p_EC, h_EC, stats_EC] = ranksum(median(data_EC_GA,2), median(data_EC_GB,2), 'tail', 'right', 'alpha', 0.01);
    test_EC = 1;
end

[p_PCMC_GA, H_PCMC_GA, ~] = ranksum(mean(data_PC_GA,2), mean(data_MC_GA,2), 'tail', 'right', 'alpha', 0.01);
...
for i = 1:numel(pos_1)
    text(pos_1(i), mean(mean_GA(:,i)) + 0.03 * range(ylim), sprintf('\\mu = %.2f', mean(mean_GA(:,i))), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontSize', 12, 'Color', 'b');
end
for i = 1:numel(pos_2)
    text(pos_2(i), mean(mean_GB(:,i)) + 0.03 * range(ylim), sprintf('\\mu = %.2f', mean(mean_GB(:,i))), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontSize', 12, 'Color', 'r');
end
