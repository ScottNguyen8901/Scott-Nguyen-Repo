function [x, x_all, counts] = BFGS (fun, G_fun, initial_x, initial_H, vec_eps)
%
...
% --- added counter (H is 0 for BFGS inverse update) ---
counts = struct('f',0,'g',1,'H',0);  % counts exclude evaluations inside line_search
...
while (vec_norm(g_k)>vec_eps) && (k<Max_iterations)
  ...
  % ---- minimal addition: ensure descent direction ----
  if (g_k.'*p_k) >= 0
      p_k = -g_k;
  end
  
  % Build phi, phi':
  f_k     = fun(x_k);           counts.f = counts.f + 1;
  phi     = @(a) fun(x_k + a*p_k);
  ...
  g_k_plus_1 = G_fun(x_k_plus_1); counts.g = counts.g + 1;
  s_k = x_k_plus_1 - x_k;
  y_k = g_k_plus_1 - g_k;  

  % ---- minimal replacement: curvature guard ----
  sty = (y_k.')*s_k;
  if sty <= 1e-12
      % skip update to keep H_k positive definite
      H_k_plus_1 = H_k;
  else
      rho_k = 1 / sty;
      % Update the inverse:
