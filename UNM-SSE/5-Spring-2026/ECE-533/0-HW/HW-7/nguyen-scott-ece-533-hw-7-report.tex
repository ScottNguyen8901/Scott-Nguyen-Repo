...
The outputs are much more varied across runs than with \texttt{top\_p = 0.1}. This makes the text more diverse and creative, but some outputs are less coherent and feel more random.
...
\item \textbf{TransformerEncoderLayer}

\begin{itemize}
    \item \textbf{Input dimensions:}
    \[
    (\text{seq\_len}, \text{batch\_size}, d_{\text{model}})
    \]
    \item \textbf{Output dimensions:}
    \[
    (\text{seq\_len}, \text{batch\_size}, d_{\text{model}})
    \]
    \item \textbf{Equation:}
    \[
    X_1 = \mathrm{LayerNorm}\bigl(X + \mathrm{MHA}(X,X,X)\bigr)
    \]
    \[
    X_2 = \mathrm{LayerNorm}\bigl(X_1 + \mathrm{FFN}(X_1)\bigr)
    \]
    where
    \[
    ...
    \begin{itemize}
        \item Multi-head attention projection weights and biases
        \item Output projection weight and bias
        \item Feedforward weights $W_1,W_2$ and biases
        \item LayerNorm scale and shift parameters
    \end{itemize}
\end{itemize}

\item \textbf{TransformerDecoderLayer}

\begin{itemize}
    \item \textbf{Input dimensions:}
    \[
    \text{target: } (\text{tgt\_len}, \text{batch\_size}, d_{\text{model}})
    \]
    \[
    \text{memory: } (\text{src\_len}, \text{batch\_size}, d_{\text{model}})
    \]
    \item \textbf{Output dimensions:}
    \[
    (\text{tgt\_len}, \text{batch\_size}, d_{\text{model}})
    \]
    \item \textbf{Equation:}
    \[
    Y_1 = \mathrm{LayerNorm}\bigl(Y + \mathrm{MHA}(Y,Y,Y)\bigr)
    \]
    \[
    Y_2 = \mathrm{LayerNorm}\bigl(Y_1 + \mathrm{MHA}(Y_1,M,M)\bigr)
    \]
    \[
    Y_3 = \mathrm{LayerNorm}\bigl(Y_2 + \mathrm{FFN}(Y_2)\bigr)
    \]
    where $M$ is the encoder output.
    \item \textbf{Learnable parameters:}
    \begin{itemize}
        \item Self-attention projection weights and biases
        \item Cross-attention projection weights and biases
        \item Output projection weights and biases
        \item Feedforward weights and biases
        \item LayerNorm scale and shift parameters
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    The encoder is a stack of $L$ encoder layers:
    \[
    H^{(0)} = X,\qquad H^{(\ell)} = \mathrm{EncoderLayer}^{(\ell)}(H^{(\ell-1)}),\quad \ell=1,\dots,L
    \]
    \[
    \mathrm{Encoder}(X)=H^{(L)}
    \]
    \item \textbf{Learnable parameters:}

    All learnable parameters from each encoder layer in the stack, and any final normalization parameters if included.
\end{itemize}

\item \textbf{TransformerDecoder}
...
\begin{itemize}
    ...
    The decoder is a stack of $L$ decoder layers:
    \[
    G^{(0)} = Y,\qquad G^{(\ell)} = \mathrm{DecoderLayer}^{(\ell)}(G^{(\ell-1)}, M),\quad \ell=1,\dots,L
    \]
    \[
    \mathrm{Decoder}(Y,M)=G^{(L)}
    \]
    \item \textbf{Learnable parameters:}

    All learnable parameters from each decoder layer in the stack, and any final normalization parameters if included.
\end{itemize}

\item \textbf{MultiheadAttention}

\begin{itemize}
    \item \textbf{Input dimensions:}
    \[
    Q:\; (\text{tgt\_len}, \text{batch\_size}, d_{\text{model}})
    \]
    \[
    K,V:\; (\text{src\_len}, \text{batch\_size}, d_{\text{model}})
    \]
    \item \textbf{Output dimensions:}
    \[
    \text{attention output: } (\text{tgt\_len}, \text{batch\_size}, d_{\text{model}})
    \]
    \[
    \text{attention weights: typically } (\text{batch\_size}, \text{tgt\_len}, \text{src\_len})
    \]
    \item \textbf{Equation:}
    \[
    Q_h = QW_h^Q,\qquad K_h = KW_h^K,\qquad V_h = VW_h^V
    \]
    \[
    ...
    \item \textbf{Learnable parameters:}
    \begin{itemize}
        \item Query projection weights and biases
        \item Key projection weights and biases
        \item Value projection weights and biases
        \item Output projection weight and bias
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Input dimensions:}
    \[
    \text{src: } (\text{src\_len}, \text{batch\_size}, d_{\text{model}})
    \]
    \[
    \text{tgt: } (\text{tgt\_len}, \text{batch\_size}, d_{\text{model}})
    \]
    \item \textbf{Output dimensions:}
    \[
    (\text{tgt\_len}, \text{batch\_size}, d_{\text{model}})
    ...
    \item \textbf{Equation:}
    \[
    M = \mathrm{Encoder}(\mathrm{src})
    \]
    \[
    \mathrm{Transformer}(\mathrm{src},\mathrm{tgt})=\mathrm{Decoder}(\mathrm{tgt},M)
    \]
    \item \textbf{Learnable parameters:}

    All learnable parameters in the encoder stack and decoder stack, including:
    \begin{itemize}
        \item all multi-head attention weights and biases,
        \item all feedforward weights and biases,
        \item all LayerNorm parameters.
    \end{itemize}
\end{itemize}

\end{enumerate}

\textbf{Note:} If \texttt{batch\_first=True} is used, the dimensions are written as
\[
(\text{batch\_size}, \text{seq\_len}, d_{\text{model}}).
\]
...
\item \textbf{Inputs used for training:}

The model is trained using:
\begin{itemize}
    \item Source sequence (\texttt{src}) — input sentence (e.g., English)
    \item Target sequence (\texttt{tgt}) — shifted target sentence (e.g., French)
\end{itemize}
The decoder receives the target sequence shifted right (teacher forcing), and the model predicts the next token.

\item \textbf{Optimizer:}

The optimizer used is Adam:
\[
\texttt{torch.optim.Adam(model.parameters(), lr=...)}
\]
It is set up with the model parameters and a learning rate, allowing all learnable parameters in the transformer to be updated during training.

\item \textbf{Number of epochs:}

The model is trained for multiple epochs (typically around 10 epochs in the notebook), iterating over the dataset several times to improve performance.
...
\item \textbf{Inputs and computed outputs:}

The deployed model takes a source sequence (\texttt{src}) as input and generates a target sequence token-by-token. At each step, the decoder outputs a probability distribution over the vocabulary, and the next token is selected (e.g., via greedy decoding or sampling). The final output is the generated translated sentence.

\item \textbf{Connection between encoder and decoder:}

The encoder processes the source sequence to produce a memory representation. The decoder uses this memory through cross-attention:
\[
\mathrm{MHA}(Q,K,V) = \mathrm{MHA}(\text{decoder states}, \text{encoder outputs}, \text{encoder outputs})
\]
Here, the encoder outputs serve as keys and values, allowing the decoder to attend to relevant parts of the input sequence when generating each token.
...
\item \textbf{PatchEmbedding}

\begin{itemize}
    \item \textbf{Input dimensions:}
    \[
    (\text{batch\_size}, C, H, W)
    \]
    \item \textbf{Output dimensions:}
    \[
    (\text{batch\_size}, N, d_{\text{model}})
    \]
    where $N = \frac{H}{P}\frac{W}{P}$ is the number of patches and $P$ is the patch size.
    \item \textbf{Equation:}

    The image is split into patches, flattened, and projected:
    \[
    x_p = \text{flatten}(\text{patch}_p)
    \]
    \[
    z_p = x_p W + b
    \]
    where $W$ maps each patch to $d_{\text{model}}$.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Equation:}
    \[
    X_1 = \mathrm{LayerNorm}(X + \mathrm{MHA}(X,X,X))
    \]
    \[
...
\begin{itemize}
    ...
    \item \textbf{Output dimensions:}
    \[
    (\text{batch\_size}, \text{num\_classes})
    \]
    \item \textbf{Equation:}

    First apply patch embedding:
    \[
    Z_0 = [x_{\text{class}}; z_1; \dots; z_N] + E_{\text{pos}}
    \]

    Then pass through encoder layers:
    \[
    Z_L = \mathrm{Encoder}(Z_0)
    \]
    ...
    y = \mathrm{MLP}(Z_L^{(0)})
    \]
    where $Z_L^{(0)}$ is the class token.
\end{itemize}
...
Below I comment on the 12 DINO attention maps (read left-to-right, top-to-bottom).

\begin{enumerate}[label=\arabic*.]
    \item \textbf{Mostly correct:} high attention on the left cat’s torso and the right cat’s face/neck. \textbf{Wrong:} small background spillover on the blanket.
    \item \textbf{Correct:} attention highlights both cats, especially the right cat’s head and the left cat’s upper body. \textbf{May be correct:} slight attention on nearby blanket folds.
    \item \textbf{Correct:} strong segmentation of the left cat’s full body and part of the right cat’s body. \textbf{Wrong:} misses some of the right cat’s lower body.
    \item \textbf{May be correct:} both cats are visible, but there is also strong attention on the background and couch, so segmentation is less clean.
    \item \textbf{Correct:} left cat is well highlighted, and the right cat’s head/neck region is identified. \textbf{Wrong:} extra attention leaks to the lower-right blanket.
    \item \textbf{Correct:} strong attention on most of the right cat and much of the left cat. \textbf{May be correct:} some attention on the remote and nearby edges.
    \item \textbf{Correct:} attention captures the left cat and the right cat’s head. \textbf{Wrong:} some isolated background hotspots.
    \item \textbf{Correct:} both cats are strongly highlighted, especially their bodies. This is one of the best segmentation maps. \textbf{Wrong:} minor spillover to the lower background.
    \item \textbf{Correct:} attention focuses on the main body regions of both cats. \textbf{Wrong:} a few extra bright spots appear off the cats.
    \item \textbf{Correct:} left cat is segmented well and the right cat is partially segmented. \textbf{May be correct:} the remote receives some attention because it is close to the cats.
    \item \textbf{May be correct:} the map finds parts of both cats, but also attends to larger blanket regions, so the segmentation is less precise.
    \item \textbf{Correct:} the right cat’s face and torso are highlighted, and part of the left cat is also detected. \textbf{Wrong:} some body regions are missed, especially on the left cat.
\end{enumerate}

Overall, the best heads are the ones where high attention stays concentrated on the cats’ bodies, faces, and legs. The weaker heads are those where attention spreads onto the blanket, couch, or remote. Some regions marked outside the cats may still be \emph{possibly correct} if they lie on boundaries, shadows, or nearby object edges that help define the cats’ shapes.
...
\begin{algorithm}[H]
	\caption{Expanded Contrastive Transforms}
	\begin{algorithmic}[1]
		\State \textbf{Input:} Image $x$
		\State \textbf{Output:} Augmented image $\tilde{x}$
		
		\Statex
		
		\State \textbf{i. Larger variety of scalings}
		\State Choose scaling transform $T_s$ from:
		\State \quad RandomResizedCrop$(96,\ \text{scale}=(0.08,1.0))$
		\State \quad RandomResizedCrop$(96,\ \text{scale}=(0.2,0.8))$
		\State \quad RandomResizedCrop$(96,\ \text{scale}=(0.5,1.0))$
		\State \quad Resize$(\text{random size in }[80,140])$
		\State \quad RandomCrop$(96)$ or CenterCrop$(96)$
		\State Apply $x \leftarrow T_s(x)$
		\State Apply RandomHorizontalFlip$(p=0.5)$
		
		\Statex
		
		\State \textbf{ii. Different color variations}
		\State Choose color transform $T_c$ from:
		\State \quad ColorJitter$(b,c,s,h)$ with random parameters
		\State \quad RandomGrayscale$(p=0.2)$
		\State \quad RandomSolarize$(\text{threshold})$
		\State \quad RandomPosterize$(\text{bits})$
		\State \quad RandomAutocontrast()
		\State \quad RandomEqualize()
		\State Apply $x \leftarrow T_c(x)$
		
		\Statex
		
		\State \textbf{iii. Different blurring functions}
		\State Choose blur transform $T_b$ from:
		\State \quad GaussianBlur$(k)$
		\State \quad MedianBlur$(k)$
		\State \quad MotionBlur$(k,\theta)$
		\State \quad BoxBlur$(k)$
		\State Apply $x \leftarrow T_b(x)$
		
		\Statex
		
		\State Convert to tensor and normalize:
		\State $x \leftarrow \text{ToTensor}(x)$
		\State $x \leftarrow \text{Normalize}(x)$
		
		\State \Return $\tilde{x} = x$
	\end{algorithmic}
\end{algorithm}
...
\begin{enumerate}[label=\roman*.]
	...
	\item \textbf{Random resizing:}  
	Random resizing alone may not be effective for contrastive learning because it does not significantly change the semantic content or viewpoint of the image. If two augmented views differ only by scale, the model can rely on low-level features (such as texture or color distribution) to match them, instead of learning meaningful, high-level representations. This weak augmentation reduces the difficulty of the contrastive task and can lead to poor generalization.
	
	\item \textbf{Random resized cropping:}  
	Random resized cropping can fail if the crops are too aggressive or poorly chosen. In such cases, two crops from the same image may not share sufficient overlapping semantic content (e.g., different parts of an object or entirely different objects in the scene). This creates false positive pairs, where the model is forced to treat unrelated views as similar. As a result, the learned representations may become noisy or less discriminative.
...
For one positive pair and $2N-2$ negative examples, this becomes
...
where $s_p$ is the positive similarity score, $s_n$ is the negative similarity score, and $\tau$ is the temperature.

For the ideal case, use

\[
s_p = 1, \qquad s_n = -1.
\]

Example calculation for $N=2$ and $\tau=1$:
...
\begin{center}
	\begin{minipage}[t]{0.48\textwidth}
		...
		\begin{tabular}{|c|c|c|c|}
			\hline
			\multicolumn{4}{|c|}{\textbf{Ideal Cases}} \\
			\hline
			$N$ & $\tau$ & $s_p$ & Loss \\
			\hline
			2 & 1 & 1.0 & 0.2395 \\
	...
	\begin{minipage}[t]{0.48\textwidth}
		...
		\begin{tabular}{|c|c|c|c|}
			\hline
			\multicolumn{4}{|c|}{\textbf{Realistic Cases}} \\
			\hline
			$N$ & $\tau$ & $s_p$ & Loss \\
			\hline
			2 & 1 & 0.9 & 0.5951 \\
...
As the temperature $\tau$ decreases, the softmax becomes sharper. This makes the model focus more strongly on the highest similarity scores, so good positive pairs receive much smaller loss. As $\tau$ increases, the softmax becomes flatter, meaning the model treats positive and negative examples more similarly. This usually increases the loss and makes contrastive learning less discriminative. Also, as $N$ increases, there are many more negative examples in the denominator, so the loss becomes larger.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		\node[box] (image) at (0,0) {Input Image\\$x$};
		
		\node[box] (aug1) at (-3,-1.8) {Random\\Augmentation 1\\$\tilde{x}_i$};
		\node[box] (aug2) at (3,-1.8) {Random\\Augmentation 2\\$\tilde{x}_j$};
		
		\node[box] (enc1) at (-3,-3.3) {Base Encoder\\$f(\cdot)$};
		\node[box] (enc2) at (3,-3.3) {Base Encoder\\$f(\cdot)$};
		
		\node[box] (proj1) at (-3,-4.8) {Projection Head\\$g(\cdot)$};
		\node[box] (proj2) at (3,-4.8) {Projection Head\\$g(\cdot)$};
		
		\node[box] (z1) at (-3,-6.3) {Latent Vector\\$z_i$};
		\node[box] (z2) at (3,-6.3) {Latent Vector\\$z_j$};
		
		\node[box, minimum width=4.2cm] (loss) at (0,-7.9) {
			InfoNCE Loss\\
			maximize positive similarity\\
			minimize negative similarity
		};
		
		\draw[arrow] (image) -- (aug1);
		\draw[arrow] (image) -- (aug2);
		
		\draw[arrow] (aug1) -- (enc1);
		\draw[arrow] (aug2) -- (enc2);
		
		\draw[arrow] (enc1) -- (proj1);
		\draw[arrow] (enc2) -- (proj2);
	...
	\end{tikzpicture}
	\caption{Block diagram of the SimCLR pre-training architecture.}
	\label{fig:simclr_pretraining}
\end{figure}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		\node[box] (encoder) at (0,-1.6) {Pre-trained Base Encoder\\$f(\cdot)$};
		
		\node[box] (features) at (0,-3.2) {Feature Vector\\$h$};
		
		\node[box] (classifier) at (0,-4.8) {Linear Classifier\\Logistic Regression};
		
		\node[box] (output) at (0,-6.4) {Class Prediction\\$\hat{y}$};
	...
	\end{tikzpicture}
	\caption{Block diagram of the classification architecture using the pre-trained SimCLR encoder.}
	\label{fig:simclr_classification}
\end{figure}
...
\begin{enumerate}[label=\roman*.]
	...
	\item \textbf{Ideal similarity (matching caption):}  
	The similarity should be as high as possible. Since CLIP uses normalized embeddings and dot products (cosine similarity), the ideal value is close to $1$, meaning the image and text embeddings are perfectly aligned.
	
	\item \textbf{Ideal similarity (non-matching caption):}  
	The similarity should be as low as possible. Ideally, this would be close to $-1$, indicating that the embeddings are in opposite directions and completely unrelated.
	
	\item \textbf{Realistic similarity (matching caption):}  
	In practice, similarities are typically less than perfect. A realistic value for a correct image-caption pair is around $0.7$ to $0.9$, indicating strong but not perfect alignment due to noise, ambiguity, and imperfect representations.
	
	\item \textbf{Realistic similarity (non-matching caption):}  
	For incorrect pairs, similarities are usually near $0$, meaning the embeddings are roughly orthogonal and share little semantic meaning. Values may range from slightly negative to slightly positive (e.g., $-0.2$ to $0.2$), depending on partial or coincidental feature overlap.
